...
def next_batch(num_way, batch_size):
  batch_images = []
  batch_labels = []
  while len(batch_images) < batch_size:
    batch = mnist.train.next_batch(1)
    if np.argmax(batch[1]) < num_way:
      batch_images.append(np.reshape(batch[0][0], (28, 28, 1)))
      batch_labels.append(np.array(batch[1][0])[:num_way])
...
conv1 = snt.Conv2D(output_channels=16,  kernel_shape=3, use_bias=False, name='conv1')
conv2 = snt.Conv2D(output_channels=32, kernel_shape=3, use_bias=False, name='conv2')
conv3 = snt.Conv2D(output_channels=64, kernel_shape=3, use_bias=False, name='conv3')
conv4 = snt.Conv2D(output_channels=64, kernel_shape=3, use_bias=False, name='conv4')
conv5 = snt.Conv2D(output_channels=50, kernel_shape=3, use_bias=False, name='conv5')
outputs = conv5(tf.nn.relu(conv4(conv3(tf.nn.relu(conv2(tf.nn.relu(conv1(inputs))))))))
outputs = Layers.global_pool(outputs)
predictions = tf.reshape(outputs, [-1, num_classes])
...
for i in range(2):
  imgs, lbls = next_batch(num_way=num_classes, batch_size=1024)
  l, _ = sess.run([loss, train_op], {inputs: imgs, targets: lbls})
  if i % 100 == 0:
    print(l)

# sess.run the weights, create a new model with them?




old_output_weights = conv3.get_variables()[0]
new_weights_shape = old_output_weights.shape.as_list()[:-1] + [additional_classes]
additional_weights = tf.random_normal(new_weights_shape)
replacement_weights = tf.Variable(sess.run(tf.concat([old_output_weights, additional_weights], -1)))
...
outputs = tf.nn.conv2d(almost_outputs, replacement_weights, [1, 1, 1, 1], padding='SAME', name='conv3')
outputs = Layers.global_pool(outputs)
predictions = tf.reshape(outputs, [-1, num_classes + additional_classes])
print(predictions.shape)
grads_weights = opt.compute_gradients(predictions)
